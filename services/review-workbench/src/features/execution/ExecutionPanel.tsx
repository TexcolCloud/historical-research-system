...
import { ExecutionCanvas } from "@/components/execution/ExecutionCanvas.tsx";
import { Button } from "@/components/ui/button.tsx";
import { client } from "@/client/api.ts";
import { executionGraph } from "@/features/execution/execution-graph.ts";
...
export default function ExecutionPanel({ bookId }: { bookId: string }) {
  ...
  const waiting =
    ...
    ["model_transport_wait", "retrieval_wait", "vision_service_wait"].includes(String(activeRun.error?.code));
  ...
  return (
    <section className="platform-executions">
      ...
      {waiting && (
        <p role="status">
          等待{activeRun?.error?.code === "retrieval_wait" ? "检索" : activeRun?.error?.code === "vision_service_wait" ? "本地视觉" : "文本"}服务恢复，已完成结果保留。
