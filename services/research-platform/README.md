...
3. 综合失败允许一轮补证。首次研究和补证各按支持、反证、限定保留一个查询名额（改写复用该用途原查询），最多 3 次不同原文读取，程序预读计入该名额；章节空召回最多增加一次全书检索。单轮累计原文预算为 10000 估算 tokens，不截断原子表格。预装配超限时让 Agent 选择较小来源包；必需原文仍超限或累计读取预算耗尽时进入已有主题拆分。
4. 候选仍须通过引用精确定位、完整主题来源核验、本地原图核验和最终核验。证据不足保留待补证问题；目录或过渡文字属于阅读覆盖范围，不要求逐段生成卡片。

检索回执绑定书籍、来源快照、索引代次与校准策略。活动重试复用已完成查询和原文读取；索引或策略改变时停止混用，需新建任务使用新版本。查询意图先保存，检索服务异常使用独立检查点冷却恢复；恢复前先补齐中断查询，不调用模型重新规划。单查询每个显式重试周期最多 12 次失败，等待按 60、120、240、300 秒封顶，阶段仍受累计 1 小时/12 次等待限制；鉴权、参数和来源版本错误不会转为服务等待。空结果不代表服务故障，也不证明不存在相关史料。执行图展示实际工具调用、检索目的、命中范围和回执。已有完成候选继续保留，不自动重制；要比较新旧方案，应创建新制卡任务。
