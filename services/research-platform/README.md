...
制卡规划、综合和文本核验默认使用 DeepSeek V4.1 Flash（API 名称 `deepseek-flash`）；逐段阅读使用同一模型。显式选择推理模型的调用保留 high/32000 输出上限，默认阅读调用保留 low/16000，职责不再由两个模型名称是否相等判断。推理档也进入回执输入摘要，避免跨预算复用。原件核验继续使用本地 Qwen3-VL-8B，不自动回退到 Pro。当前别名说明见 [DeepSeek 官方发布说明](https://deepseek.com/news/deepseek-v4-1-flash/)。
