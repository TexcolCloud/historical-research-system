...
from hrs_platform.services.card_evidence import PURPOSES, CardEvidence, EvidenceQuery
...
from hrs_platform.services.reading import (
    card_model,
...
class Cards:
    ...
    async def generate(self, run_id, *, incremental=False):
        ...
        RunLifecycle(self.engine).transition(run_id, "processing", "planning")
        ...
        RunLifecycle(self.engine).transition(run_id, "processing", "vision")
    ...
    async def finalize(self, run_id, batch_key=None):
        ...
        if self.outputs.get(run_id, final_stage):
            return {"run_id": run_id}
        RunLifecycle(self.engine).transition(run_id, "processing", "finalization")
