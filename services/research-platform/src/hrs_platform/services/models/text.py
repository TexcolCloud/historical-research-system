...
from hrs_platform.services.runs.outputs import Outputs, StageInputMismatch, execution_parent, fingerprint
    ...
    async def _attempt(
        ...
        *,
        ...
        if not self.settings.deepseek_api_key:
            raise TaskError("尚未配置 DeepSeek 文本模型密钥。", non_retryable=True)
        attempt = 1
        ...
        async def record_request(request):
            ...
            try:
                ...
                if (
                    not requests
                    and isinstance(error, TaskError)
