...
from hrs_platform.services.runs.outputs import execution_parent, fingerprint
...
class CardEvidence:
    ...
    def pin(self, run, snapshot):
        ...
        if not generation or not parent["result"].get("published"):
            raise TaskError("本书尚无已发布检索索引，请完成索引后重试。", type="card_index_missing")
        ...
        if committed != generation:
            raise TaskError("索引版本尚未完成，保留制卡进度等待重建。", type="card_index_missing")
        if not self.search.client.indices.exists(index=self.settings.opensearch_index):
            raise TaskError("检索索引不存在，请恢复索引后重试。", type="card_index_missing")
        ...
        if not count:
            raise TaskError("本书索引内容缺失，请恢复索引后重试。", type="card_index_missing")
        ...
        async def perform_query(index):
            ...
            with self.outputs.operation(
                ...
                if not await asyncio.to_thread(self.search.client.indices.exists, index=corpus["index"]):
                    raise TaskError("检索索引不可用，保留进度等待恢复。", type="card_index_missing")
                metrics = {}
