...
- After OCR produces Markdown, run full local visual review. Apply uniquely located model corrections only after a separate original-image check of the corrected page completes with no unresolved content; retain original text, patches and evidence hashes as machine decisions. Unreadable, uncertain, unlocated or failed corrections remain for human review. Preserve existing human decisions and drafts. Library ingestion and downstream partitioning wait until the whole document has no unresolved or unreviewed content; clean blocks remain readable in the review workspace. Preserve table and figure originals for checking when automatic review fails. Existing published records are not retroactively deleted.
